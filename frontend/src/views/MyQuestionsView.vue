...
const headerAvatarUrl = computed(() => authStore.currentUser?.avatar_url || '')
...
function parseEmbeddedQuestion(row: Question) {
  ...
  for (const value of candidates) {
    ...
    try {
      ...
      // Ignore legacy malformed rows and fall back to the original fields.
    }
  }
  return null
}
...
async function removeQuestion(row: Question) {
  await ElMessageBox.confirm(`确定删除题目“${getQuestionText(row).slice(0, 30)}...”吗？`, '确认删除', {
    type: 'warning',
  })
...
function openDuplicateDialog() {
  duplicateGroups.value.forEach((group) => {
    if (!duplicateSelections[group.question]) {
      duplicateSelections[group.question] = group.items[0]?.id
    }
  })
...
async function resolveDuplicateGroup(group: DuplicateQuestionGroup) {
  ...
  await resolveDuplicateQuestionApi({
    keep_id: keepId,
    remove_ids: group.items.filter(item => item.id !== keepId).map(item => item.id),
  })
...
function buildXlsxBlob(rows: Question[]) {
  ...
  const sheetRows = [
    ...
    ...rows.map((item, index) => {
      ...
      return `<row r="${index + 2}">${values.map(cell).join('')}</row>`
    }),
  ].join('')
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets>
    <sheet name="我的题库" sheetId="1" r:id="rId1"/>
  </sheets>
</workbook>`,
    },
    {
...
async function exportQuestions() {
  ...
  try {
    const pageSize = 100
    const first = await getQuestionBankApi({
      page: 1,
      page_size: pageSize,
      mine: true,
      keyword: searchKeyword.value || undefined,
    ...
    const blob = buildXlsxBlob(exportRows)
...
<template>
  <div class="my-question-page">
    <header class="my-question-header">
    ...
    <main class="my-question-content">
      <div class="page-header">
        ...
        <div class="page-actions">
          <el-button :icon="'Download'" :loading="exporting" @click="exportQuestions">导出 Excel</el-button>
      ...
      <el-table :data="questions" v-loading="loading" stripe>
        ...
        <el-table-column label="来源" width="110">
          <template #default="{ row }">
            {{ sourceMap[row.source || ''] || row.source || '未知' }}
          </template>
        </el-table-column>
            ...
            <el-radio-group v-model="duplicateSelections[group.question]" class="duplicate-options">
              ...
              >
                <div class="duplicate-option">
                  <strong>保留记录 #{{ item.id }}</strong>
                  <span>{{ getAnswerText(item) || '未填写答案' }}</span>
                  <small>{{ sourceMap[item.source || ''] || item.source || '未知' }} · {{ item.created_at }}</small>
                </div>
              </el-radio>
            </el-radio-group>
            <div class="duplicate-actions">
              <el-button type="primary" size="small" @click="resolveDuplicateGroup(group)">保留所选，删除其他</el-button>
            </div>
          </section>
        </div>
...
.duplicate-options :deep(.el-radio) {
  height: auto;
  margin-right: 0;
  padding: var(--spacing-sm);
}

.duplicate-options :deep(.el-radio__label) {
  white-space: normal;
  width: 100%;
}
