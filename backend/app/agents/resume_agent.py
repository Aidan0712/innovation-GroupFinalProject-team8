...
class ResumeAgent(BaseAgent):
    ...
    async def evaluate(
        ...
        user_text = "请按照规定的 Markdown 格式评估这份简历。"
        if target_position:
            user_text = f"请评估这份简历与「{target_position}」岗位的匹配度，按照 Markdown 格式输出评估报告。"
    ...
    async def evaluate_stream(
        ...
        user_text = "请按照规定的 Markdown 格式评估这份简历，直接输出格式化的评估报告。"
        if target_position:
            user_text = f"请评估这份简历与「{target_position}」岗位的匹配度，按照规定的 Markdown 格式直接输出评估报告。"
